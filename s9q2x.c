...
void sumSubset(int k, int currentSum, int remainingSum) {
    subset[k] = 1;
    if (currentSum + k + 1 == sum) {
        displaySubset();
    } else if (currentSum + k + 1 < sum) {
        sumSubset(k + 1, currentSum + k + 1, remainingSum - (k + 1));
    }
    subset[k] = 0;
    if (currentSum + remainingSum - k >= sum) {
        sumSubset(k + 1, currentSum, remainingSum - k);
    }
}
